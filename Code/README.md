...
The library sub-classes the Adafruit_GFX library and, as a result, it is possible to use the GFX drawing and text routines in addition to hardware specific routines.
...
```  
This is an override for the Adafruit_GFX library drawPixel() mthod. It allows the Adafruit_GFX library to draw into our pixel array. See setPixel() and getPixel().
...
```
This is a led test , regardless of programming this truens on ALL leds if state=true and off if state=false.

It's usefult to check that you don't have any burned out LEDS
...
The range of level is 0-32. begin() sets the intial intensity to 5
...
void dumpPixels();
```
A debugging method - outputs the contents of the pixel array to the serial monitor port as a series of 1's and spaces. Spaces represent LEDs in the off state.
...
Each AS1100 chip controls 8 digit lines (rows) and 6 segment lines (columns). The six segment lines are stored in the pixel array hence it is an array of uint8_t. In order to accomodate variable length panels the array is organised as 8 rows of arrays of uint8_t of which only the 6 bottom bits are used - corresponding to the 6 segment outputs of the AS1100.
